...
/**
 * result enum
 * "@SuppressWarnings" 可用参数
 * "all"：抑制所有类型的警告。
 * "boxing"：抑制与自动装箱和拆箱相关的警告。
 * "cast"：抑制与类型转换相关的警告。
 * "dep-ann"：抑制使用过时的注解相关的警告。
 * "deprecation"：抑制使用过时的方法或类相关的警告。
 * "fallthrough"：抑制 switch 语句中缺少 break 的警告。
 * "finally"：抑制 finally 块无法正常完成的警告。
 * "hiding"：抑制隐藏变量的警告。
 * "incomplete-switch"：抑制 switch 语句中缺少某些 case 的警告。
 * "nls"：抑制非本地化字符串相关的警告。
 * "null"：抑制可能存在空指针异常的警告。
 * "rawtypes"：抑制使用原始类型相关的警告。
 * "resource"：抑制未关闭资源相关的警告。
 * "restriction"：抑制不符合访问限制的警告。
 * "serial"：抑制没有序列化 ID 的警告。
 * "static-access"：抑制静态访问非静态成员的警告。
 * "static-method"：抑制非静态方法从静态上下文中访问的警告。
 * "super"：抑制对超类方法的不正确调用的警告。
 * "synthetic-access"：抑制对合成字段或方法的访问的警告。
 * "sync-override"：抑制未同步覆盖同步方法的警告。
 * "unchecked"：抑制未检查操作相关的警告。
 * "unqualified-field-access"：抑制对未限定字段的访问的警告。
